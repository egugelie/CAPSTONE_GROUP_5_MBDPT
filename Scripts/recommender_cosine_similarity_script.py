...
def recommender_cosine_similarity(cart_articles, orders, embeddings, embeddings_matrix):
    ...
    if not productos_recomendados:
        print("No recommendations (Cosine)")
    else:
        print('Customers with preferences similar to yours also added these items to their carts: (Cosine)')
        for product in productos_recomendados[:10]:
            print(f"- {product}")