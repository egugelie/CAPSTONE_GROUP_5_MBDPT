...
def predict_cluster(cart):
    ...
    # Ensure we only have the list of product names
    recommended_product_names = [product for product in recommended_products if isinstance(product, str)]
    
    # Filter out the products already in the cart and get top 10
    top_10_recommended_products = [product for product in recommended_product_names if product not in cart][:10]
    
    print("Based on your behaviour, other customers also bought:")
    return top_10_recommended_products
